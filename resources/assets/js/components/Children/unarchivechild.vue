				...
				<div v-if="child_id && activeOrderItems.length">
					<div
						...
						<p class="fs-no fg tr" v-if="Order.order.order_status == 'fulfilled'">
							{{ Order.order.updated_at | formatDate("MM,DD,YYYY") }}
						</p>
						<p class="fs-no fg tr" v-else>
							{{ Order.order.created_at | formatDate("MM,DD,YYYY") }}
						</p>
					</div>
				</div>
				<div v-else-if="child_id">
...
export default {
	...
	methods: {
		...
		loadChild() {
            ...
            let cid = this.child_id;
				axios.post('/api/child/unarchive',{cid})
				.then( (response) => {
		...
		unarchiveChild() {
			...
			axios.put('/api/child/update',{cid})
			.then( (response) => {
				if (response.data.success) {
					this.$toast.success({
						title: "Success",
