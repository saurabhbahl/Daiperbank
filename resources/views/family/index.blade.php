...
<div class="flex-auto flex justify-start content-stretch o-hidden pxr">
	<div class="col-xs-7 pa0 bg-white br b--black-20 flex-auto o-hidden flex flex-column justify-stretch">
		...
		<child-list ref="child-list" class="child-list fg fs oy-auto" :initial-children="{{ json_encode($Children->items()) }}">

			<template slot="item" slot-scope="Child">
				<a class="view-child-detail db flex bb b--black-20 pa3 a-plain a:hov-nou hov:bg-washed-blue clickable" href="#" :data-id="Child.id" :key="Child.id">
		...
		<? if ($Children->lastPage() > 1) : ?>
			<div class="fg-no fs-no pv3 flex justify-center bg-washed-blue bt b--black-20">
				<a href="<?= $Children->previousPageUrl() ?? "#"; ?>" class="btn btn-default mr <?= !$Children->previousPageUrl() ? 'mod-disabled' : ''; ?>">
				...
				<? foreach (range($start_page, $end_page) as $pg) : ?>
					<a href="<?= $Children->url($pg); ?>" class="btn btn-default mr <?= $pg == $Children->currentPage() ? 'b' : ''; ?>">
						<?= $pg; ?>
					</a>
				<? endforeach; ?>

				<a href="<?= $Children->nextPageUrl() ?? "#"; ?>" class="btn btn-default <?= !$Children->nextPageUrl() ? 'mod-disabled' : ''; ?>">
					Next Page
					<i class="fa fa-chevron-right"></i>
				</a>
			</div>
	...
	<div class="col-xs-5 ">
		...
		<div class="mv4 bg-white br3 py4 px4">
			<form method="get">
				<div class="mb">
					<p class="b">Search</p>
					<input name="filter[search]" type="search" class="form-control" placeholder="Search for children..." value="<?= e(request()->input('filter.search')); ?>" />
					<p class="tr"><small><a href="#">Search tips</a></small></p>
				</div>

				<div class="mb">
					<p class="b">Parent/Guardian</p>
					<guardian-select ref="guardian-select" :guardians='<?= e($Guardians->toJson()); ?>' :default='<?= json_encode(array_map('intval', explode(',', request()->input('filter.guardian', '')))); ?>' @keyup.enter.stop.prevent="false"></guardian-select>
					<input type="hidden" id="guardian_filter" name="filter[guardian]" value="<?= request()->input('filter.guardian', ''); ?>">
				</div>
				...
				<div class="pt">
					...
					<? if (
						request()->exists('filter.search') && request()->input('filter.search')
	...
	<child-detail-pane ref="child-detail-pane" class="pxa pint pinr pinb w-40 bg-white shadow-2 flex flex-column justify-between" :orders="<?= e($DraftOrders->toJson()); ?>"></child-detail-pane>
